...
double résolution puis réduit pour lisser les traits. Les images produites sont versionnées : ce script ne sert
...
L, H = 1200, 627          # format recommandé par LinkedIn (1,91:1)
K = 2                     # suréchantillonnage
...
def relief(graine=7, echelle=430, cellule=5, n_niveaux=24):
    ...
    for l in range(1, n_niveaux + 1):
        ...
        if niveau < eau:
            continue
        maitresse = l % 5 == 0
        tracer(niveau, COURBE, 0.85 if maitresse else 0.42, 1.3 if maitresse else 0.7)
    tracer(eau, EAU, 0.75, 1.0)
    return img


# ---------- Habillage cartographique ----------

def cadre(dessin, marge=18, bande=5, pas=48):
    x0, y0, x1, y1 = marge * K, marge * K, (L - marge) * K, (H - marge) * K
    b, p = bande * K, pas * K
    dessin.rectangle([x0, y0, x1, y1], outline=ENCRE, width=K)
    dessin.rectangle([x0 + b, y0 + b, x1 - b, y1 - b], outline=ENCRE, width=K)
...
def fleche_nord(dessin, cx, haut):
    s = K
    dessin.text((cx * s, haut * s), "N", font=police("mono", 13), fill=ENCRE, anchor="mt")
    pointe, bas, milieu = (cx * s, (haut + 20) * s), (haut + 64) * s, (haut + 54) * s
    contour = [pointe, ((cx + 11) * s, bas), (cx * s, milieu), ((cx - 11) * s, bas)]
    dessin.polygon(contour, outline=ENCRE, width=round(1.5 * s))
    dessin.polygon([pointe, (cx * s, milieu), ((cx - 11) * s, bas)], fill=ENCRE)


def barre_echelle(dessin, x, y, largeur=150):
    s, n = K, 4
    for i in range(n):
        x0 = x + i * largeur / n
        dessin.rectangle([x0 * s, y * s, (x0 + largeur / n) * s, (y + 7) * s],
                         fill=ENCRE if i % 2 == 0 else PAPIER_3, outline=ENCRE, width=s)
    fonte = police("mono", 11)
    for texte, dx in (("0", 0), ("1", largeur / 2), ("2 km", largeur)):
        dessin.text(((x + dx) * s, (y + 12) * s), texte, font=fonte, fill=ENCRE_DOUCE, anchor="mt")
...
def cartouche(img, t):
    ...
    d.rectangle([(x - 6) * s, (y - 6) * s, (x + w + 6) * s, (y + h + 6) * s], outline=ENCRE, width=s)
    d.rectangle([x * s, y * s, (x + w) * s, (y + h) * s], fill=PAPIER_3, outline=ENCRE, width=s)
    ...
    d.line([(px * s, (py + 28) * s), ((x + w - 40) * s, (py + 28) * s)], fill=(210, 204, 192), width=s)

    d.text((px * s, (py + 52) * s), "Baptiste Feldmann", font=police("titre", 60), fill=ENCRE, anchor="lt")
    d.text((px * s, (py + 150) * s), t["poste"], font=police("mono", 19), fill=ACCENT, anchor="lt")
    d.text((px * s, (py + 188) * s), t["accroche"], font=police("sans", 18), fill=ENCRE_DOUCE, anchor="lt")
    ...
    d.text(((px + 26) * s, base * s), "baptistefeldmann.github.io", font=police("mono", 19), fill=ENCRE,
           anchor="lm")
    d.text(((x + w - 40) * s, base * s), t["sections"], font=police("mono", 13), fill=ENCRE_DOUCE, anchor="rm")
...
def generer(langue):
    ...
    for gx in range(0, L, 80):
        g.line([(gx * K, 0), (gx * K, H * K)], fill=ENCRE + (16,), width=K)
    for gy in range(0, H, 80):
        g.line([(0, gy * K), (L * K, gy * K)], fill=ENCRE + (16,), width=K)
    ...
    img.convert("RGB").resize((L, H), Image.LANCZOS).save(sortie, optimize=True)
